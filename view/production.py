...
try:
    with Database() as db:
        ...
        query_service = QueryService(db, production_repository)
        ...
        with tab1:
            ...
            with st.form(key="product_lot_search", clear_on_submit=True, border=False):
                ...
                if submitted:
                    products = query_service.find_product_by_product_lot(lot_no)
            ...
            with st.form(key="input_material", clear_on_submit=True, border=False):
                ...
                if submitted:
                    input_material = query_service.input_material_by_production(
                        production_id
                    )
